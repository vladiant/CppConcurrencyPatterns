...
class RingBufferV5 {
 ...
 public:
  bool push(const T& value) noexcept {
    ...
    if (new_head == tail_cached_) [[unlikely]] {  // Full
      tail_cached_ = tail_.load(std::memory_order_acquire);
      if (new_head == tail_cached_) [[unlikely]] {  // Still full
        return false;
      }
    }
  ...
  bool pop(T& value) noexcept {
    ...
    if (tail == head_cached_) [[unlikely]] {  // Empty
      head_cached_ = head_.load(std::memory_order_acquire);
      if (tail == head_cached_) [[unlikely]] {  // Still empty
        return false;
      }
    }
...
void demo_queue_full() {
  ...
  RingBufferV5<int, 5> queue;  // Small queue
