...
class LockfreeAtomicPushPopPtrSpsc : NonCopyableNonMovable {
 ...
 public:
  ...
  bool push(const T& t) {
    ...
    if (curPush - popInd_.load(std::memory_order_acquire) == cap_) return false;
    buf_[curPush % cap_] = t;
...
void demo_basic_usage() {
  ...
  LockfreeAtomicPushPopPtrSpsc<int> queue(10);
...
void demo_queue_full() {
  ...
  LockfreeAtomicPushPopPtrSpsc<int> queue(5);  // Small queue
...
void demo_producer_consumer() {
  ...
  LockfreeAtomicPushPopPtrSpsc<int> queue(100);
...
void demo_performance() {
  ...
  LockfreeAtomicPushPopPtrSpsc<int> queue(1024);
  std::atomic<bool> done{false};
