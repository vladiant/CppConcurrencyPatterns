...
class EventHandler {
 public:
  explicit EventHandler(std::unique_ptr<const Message> msg)
      : fresh_msg{std::move(msg)} {
  ...
  void OnMessage(std::unique_ptr<const Message> msg) {
    fresh_msg = std::move(msg);
  }
 ...
 private:
    ...
    while (event_loop_start) {
      if (fresh_msg) {
        ...
        std::this_thread::sleep_for(
            wait_for_input);  // Simulate waiting for input, based on the
                              // message type, by putting the thread to sleep.

        ParseMessage(std::move(fresh_msg));
      }
    }
  }

  void ParseMessage(std::unique_ptr<const Message> msg) {
    ...
    if (msg->type == static_cast<int32_t>(MessageTypes::REGISTER_HANDLER)) {
      if (this->getHandler()) {
        ...
        std::cerr << "Cannot register Event handler since it is NULL!"
               ...
               static_cast<int32_t>(MessageTypes::REMOVE_HANDLER)) {
  ...
  std::unique_ptr<const Message> fresh_msg;
...
int main() {
  ...
  std::unique_ptr<reactor::Message> msg_ptr;
  reactor::Message message1 = {
      static_cast<int32_t>(reactor::MessageTypes::OPEN), {0, 1, 2, 3}};
  msg_ptr.reset(new reactor::Message);
  *msg_ptr = message1;

  reactor::EventHandler event_handler(std::move(msg_ptr));
