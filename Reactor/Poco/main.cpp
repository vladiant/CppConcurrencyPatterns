// http://www.modernescpp.com/index.php/reactor
// https://github.com/pocoproject/cmake-sample/blob/main/CMakeLists.txt
...
class EchoHandler {
 public:
  EchoHandler(const StreamSocket& s, SocketReactor& r) : socket(s), reactor(r) {
    reactor.addEventHandler(socket, Observer<EchoHandler, ReadableNotification>(
...
class DataHandler {
 public:
  DataHandler(StreamSocket& s, SocketReactor& r)
      : socket(s), reactor(r), outFile("reactorOutput.txt") {
    reactor.addEventHandler(socket, Observer<DataHandler, ReadableNotification>(
                                        *this, &DataHandler::socketReadable));
    reactor.addEventHandler(socket, Observer<DataHandler, ShutdownNotification>(
  ...
  ~DataHandler() {
    reactor.removeEventHandler(socket,
                               Observer<DataHandler, ReadableNotification>(
                                   *this, &DataHandler::socketReadable));
    reactor.removeEventHandler(socket,
                               Observer<DataHandler, ShutdownNotification>(
                                   *this, &DataHandler::socketShutdown));
  }
  ...
  void socketShutdown(ShutdownNotification*) { delete this; }
